...
void loop() {
  ...
  if (cmd == "REBOOT") {
    ...
    if (mode != SystemMode::IDLE && mode != SystemMode::LANDED) {
      sendResponse("PARSE BLOCKED - STOP OR LAND FIRST\n");
    } else {
  ...
  // --- Pre-Flight Commands ---
    ...
    if (armSequenceActive) {
      if (cmd == "READY") {
        ...
        sendResponse("READY ACTIVE - ONLY STOP, DISPLAY OR REBOOT\n");
      }
    }
      ...
      // Warm up only once. STOP and REBOOT remain responsive during the wait.
      ...
      while (millis() - warmupStartMs < IMU_CALIB_WARMUP_MS) {
        ...
        c2.toUpperCase();
        if (c2 == "STOP") {
          sendResponse("CALIBRATION ABORTED\n");
          stopSystem();
      ...
      while (!calibrationAborted &&
             stableWindows < IMU_CALIB_STABLE_WINDOWS &&
             millis() - calibrationStartMs < IMU_CALIB_TIMEOUT_MS) {
        sendResponse("CAL WINDOW (512 samples)...\n");

        // The vehicle is in the known 90-degree vertical pose. This is the
        // only stage that estimates accelerometer bias. Isolated shocks are
        // rejected by the trimmed-mean/MAD statistics inside the driver.
        xSemaphoreTake(spiMutex, portMAX_DELAY);
        bool windowOk =
            imu.calibrate(IMU_CALIB_WINDOW_SAMPLES, false);
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 cs.mean_a[0], cs.mean_a[1], cs.mean_a[2],
                 imu.wasLastPoseValid() ? "OK" : "BAD");
        sendResponse(buf);

        if (windowOk) {
          for (int i = 0; i < 3; i++) {
            sumMeanG[i] += cs.mean_g[i];
            sumMeanA[i] += cs.mean_a[i];
          }
          stableWindows++;
          snprintf(buf, sizeof(buf), "CAL STABLE %u/%u\n",
                   stableWindows, IMU_CALIB_STABLE_WINDOWS);
          sendResponse(buf);
        } else {
          stableWindows = 0;
          ...
          sendResponse(imu.wasLastPoseValid()
                           ? "CAL UNSTABLE - RETRY WINDOW\n"
                           : "CAL POSE BAD - KEEP VERTICAL\n");
        }

        if (stableWindows >= IMU_CALIB_STABLE_WINDOWS) {
          LSM6DSO32::CalibStats finalStats = cs;
          for (int i = 0; i < 3; i++) {
            finalStats.mean_g[i] =
                sumMeanG[i] / (float)IMU_CALIB_STABLE_WINDOWS;
            finalStats.mean_a[i] =
                sumMeanA[i] / (float)IMU_CALIB_STABLE_WINDOWS;
          }
        ...
        // Check commands between short windows instead of blocking forever.
        String c2 = getIncomingRaw();
        c2.toUpperCase();
        if (c2 == "STOP") {
          sendResponse("CALIBRATION ABORTED\n");
      ...
      if (!calibrationDone && !calibrationAborted) {
        sendResponse("CALIBRATION TIMEOUT - CHECK MOUNT/POSE\n");
        beep(100, 3);
      }
    }
    else if (cmd == "ERASE") {
...
void Flight_Task(void *pvParameters) {
  ...
  static bool     stage2_ignited   = false;
  ...
  for (;;) {
    ...
    while (flightActive) {
      ...
      // STOP may arrive while this task is sleeping. Do not execute one more
      // flight-control cycle or re-assert an actuator after STOP has cleared it.
      if (!flightActive || getSystemMode() != SystemMode::FLIGHT) break;
      ...
      switch (flightPhase) {
        ...
        case COASTING: {
          ...
          // Apogee detection:
          //   1) normal KF velocity detection, debounced for 300 ms
          //   2) fallback timeout measured from actual stage-2 ignition
          {
            bool sensor_apogee =
                (vel_up < -0.5f && alt > 15.0f);
            bool timeout_apogee =
                stage2_ignited &&
                (now_ms - pyro1_start_ms >= STAGE2_APOGEE_TIMEOUT_MS);
            ...
            if (timeout_apogee || descent_count >= 3) {
              if (!stage2_attempted) {
              ...
              sendResponse(timeout_apogee ? "APG TIMEOUT\n" : "APG\n");
...
void displaySensors() {
  ...
  bool preLaunch =
      ...
      mode == SystemMode::ARMED;
  if (!preLaunch) {
    sendResponse("DISPLAY BLOCKED - PRE-LAUNCH ONLY\n");
    return;
  }
  ...
  snprintf(line, sizeof(line),
           ...
           si.ax, si.ay, si.az,
           si.gx * 180.0f / (float)M_PI,
           si.gy * 180.0f / (float)M_PI,
           si.gz * 180.0f / (float)M_PI);
  sendResponse(line);
  vTaskDelay(pdMS_TO_TICKS(10));
...
void stopSystem() {
  ...
  preTrigger.clear();
  resetLaunchDetector();
  setSystemMode(mode == SystemMode::FLIGHT
                    ? SystemMode::LANDED
                    : SystemMode::IDLE);
  xSemaphoreGive(stateMutex);
  sendResponse("STOPPED.\n");
