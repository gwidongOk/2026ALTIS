...
void PreTriggerBuffer::pushState(const State_nominal &state) {
  Record record = {};
  record.type = RECORD_STATE;
  record.state = state;
  push(record);
}

void PreTriggerBuffer::pushEvent(
    FlightPhase phase, uint8_t eventId, uint32_t timestamp) {
  Record record = {};
  record.type = RECORD_EVENT;
  record.event.timestamp = timestamp;
  record.event.phase = static_cast<uint8_t>(phase);
...
bool MX25Logger::appendPreTriggerRecord(
    const PreTriggerBuffer::Record &record) {
  if (record.type == PreTriggerBuffer::RECORD_IMU) {
    imu_pkt pkt;
    pkt.header.SYNC_BYTE = 0xAA;
    pkt.header.id        = ID_IMU;
    pkt.header.len       = sizeof(pkt);
    pkt.t  = record.imu.timestamp;
    pkt.gx = record.imu.gx;
    pkt.gy = record.imu.gy;
    pkt.gz = record.imu.gz;
    pkt.ax = record.imu.ax;
    pkt.ay = record.imu.ay;
    pkt.az = record.imu.az;
    appendRaw(reinterpret_cast<const uint8_t *>(&pkt), sizeof(pkt));
    return true;
  }

  if (record.type == PreTriggerBuffer::RECORD_BARO) {
    baro_pkt pkt;
    pkt.header.SYNC_BYTE = 0xAA;
    pkt.header.id        = ID_BARO;
    pkt.header.len       = sizeof(pkt);
    pkt.t   = record.baro.timestamp;
    pkt.alt = record.baro.alt;
    appendRaw(reinterpret_cast<const uint8_t *>(&pkt), sizeof(pkt));
    return true;
  }

  if (record.type == PreTriggerBuffer::RECORD_STATE) {
    state_pkt pkt;
    pkt.header.SYNC_BYTE = 0xAA;
    pkt.header.id        = ID_STATE;
    pkt.header.len       = sizeof(pkt);
    pkt.t = record.state.timestamp;
    memcpy(pkt.p, record.state.p, sizeof(record.state.p));
    memcpy(pkt.v, record.state.v, sizeof(record.state.v));
    memcpy(pkt.q, record.state.q, sizeof(record.state.q));
    appendRaw(reinterpret_cast<const uint8_t *>(&pkt), sizeof(pkt));
    return true;
  }

  if (record.type == PreTriggerBuffer::RECORD_EVENT) {
    event_pkt pkt;
    pkt.header.SYNC_BYTE = 0xAA;
    pkt.header.id        = ID_EVENT;
    pkt.header.len       = sizeof(pkt);
    pkt.t        = record.event.timestamp;
    pkt.phase    = record.event.phase;
    pkt.event_id = record.event.eventId;
    appendRaw(reinterpret_cast<const uint8_t *>(&pkt), sizeof(pkt));
    return true;
  }
...
void MX25Logger::serviceFlush() {
  Item item;
  size_t drained = 0;
  while (drained < SERVICE_BATCH_ITEMS &&
         xQueueReceive(_queue, &item, 0) == pdTRUE) {
