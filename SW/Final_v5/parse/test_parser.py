...
1. Build synthetic binary packets for all 4 types and flight events 1-7
...
def parse_buffer(buf: bytearray):
    ...
    while size - cursor >= 3:
        ...
        if size - cursor < plen:
            break
        raw = bytes(buf[cursor:cursor + plen])
...
tests = [
    ...
    {
        'name'  : 'EVENT (ID=6) BURNOUT',
        'raw'   : make_event(3500000, 2, 2),
        'pid'   : 6,
        'expect': {'t': 3500000, 'phase': 2, 'event_id': 2},
    },
    {
        'name'  : 'EVENT (ID=6) APOGEE',
        'raw'   : make_event(4000000, 3, 3),
        'pid'   : 6,
        'expect': {'t': 4000000, 'phase': 3, 'event_id': 3},
    },
    {
        'name'  : 'EVENT (ID=6) NSC',
        'raw'   : make_event(4500000, 2, 5),
        'pid'   : 6,
        'expect': {'t': 4500000, 'phase': 2, 'event_id': 5},
    },
    {
        'name'  : 'EVENT (ID=6) STAGE2',
        'raw'   : make_event(4750000, 2, 6),
        'pid'   : 6,
        'expect': {'t': 4750000, 'phase': 2, 'event_id': 6},
    },
    {
        'name'  : 'EVENT (ID=6) MAIN',
        'raw'   : make_event(5000000, 3, 7),
        'pid'   : 6,
        'expect': {'t': 5000000, 'phase': 3, 'event_id': 7},
    },
    {
        'name'  : 'EVENT (ID=6) LANDING',
        'raw'   : make_event(5500000, 4, 4),
        'pid'   : 6,
        'expect': {'t': 5500000, 'phase': 4, 'event_id': 4},
    },
]
...
def make_full_dump():
    ...
    dump += make_event(315000, 2, 5)             # NSC
    dump += make_event(317000, 2, 6)             # STAGE2
    dump += make_event(320000, 3, 3)             # APOGEE / DESCENT
    dump += make_event(325000, 3, 7)             # MAIN / DESCENT
...
expected_pids = [1, 2, 5, 6, 6, 6, 6, 6, 6, 6]
...
print('\n[3] Fragmented stream parsing')
fragmented_raw = make_state(
    600000, 1, 2, 3, 4, 5, 6, 1, 0, 0, 0
)
fragmented_buf = bytearray(fragmented_raw[:10])
first_pass = parse_buffer(fragmented_buf)
fragmented_buf.extend(fragmented_raw[10:])
second_pass = parse_buffer(fragmented_buf)
if not first_pass and len(second_pass) == 1 and not fragmented_buf:
    print('  PASS  incomplete packet retained and decoded after completion')
else:
    print('  FAIL  fragmented packet handling')
    all_pass = False

long_garbage = bytearray(b'\xFF' * (32 * 1024 * 1024))
long_garbage.extend(make_event(700000, 3, 7))
garbage_packets = parse_buffer(long_garbage)
if len(garbage_packets) == 1 and not long_garbage:
    print('  PASS  32 MiB erased-flash region skipped')
else:
    print('  FAIL  long erased-flash region handling')
    all_pass = False
